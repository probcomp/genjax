"""Test the repeat pattern implementation."""

import jax
import jax.numpy as jnp
import time

from core_repeat_pattern import run_repeat_gen2d_inference
from data import generate_tracking_data


def test_repeat_pattern():
    """Test the repeat pattern implementation."""
    print("=== Testing Repeat Pattern Implementation ===\n")
    ...
    # Test repeat pattern implementation
    print(f"\n--- Testing Repeat Pattern Implementation (K={n_components}) ---")
    start_time = time.time()
    try:
        particles = run_repeat_gen2d_inference(
        ...
        print(f"✅ Repeat Pattern: {elapsed_time:.2f}s")
        print(f"   Log ML: {log_ml}")
        print(f"   ESS: {ess}")

    except Exception as e:
        print(f"❌ Repeat Pattern failed: {e}")
        import traceback
    ...
    for K in [2, 4, 5]:
        start_time = time.time()
        try:
            test_particles = run_repeat_gen2d_inference(
                observations=observations,
                observation_counts=counts,
                n_components=K,
    ...
    print("\n✅ Repeat pattern tests completed!")
