...
pixi run paper-perfbench --is-frameworks genjax numpyro --hmc-frameworks genjax numpyro pyro
```

Key pipeline flags:

- `--particles` controls the IS particle grid (default 1k/5k/10k).
- `--is-repeats` / `--is-inner-repeats` default to 100×100 for the JAX-based frameworks (Pyro is automatically reduced to 5×5 unless you override the flags).
